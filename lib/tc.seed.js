...
(function(window, $) {
//NI is a namespace that we use.
	var NI = {},
	    fakeConsole;
//NI extensions
	NI.ex = {
//This is a function that checks if passed in value is string or not.
		checkStr: function(s) {
			var t = typeof s;
...
//Checking jQuery Node.
		checkJQNode: function($e, type) {
//Parameters passed in: $e for element and type for node type.
//You can see here, the element is passed in .checkJQ() function first to check
//if it is a jQeury object. Then matching type name with set of node names that 
//jQuery recognizes.
			NI.ex.checkJQ($e);
...
//So, since javascript doesn't distinguish arrays or nulls, this typeOf function
//is made to check arrays and nulls.
		typeOf: function(v) {
			var s = typeof v;
			if (s === "object") {
				if (v) {
...
//This function checks if the value is empty or not. If empty return value will be false.
		isEmpty: function(v) {
			NI.ex.checkStr(v);
			return $.trim(v) ?
				false : true;
		},
//Checking if the element is the parent node or not. If the very first instance of such 
//element has a parent node, it's not headless, returning true.
...
//getConsole is used to acquire a reference to the global console object safely,
//because sometimes the console isn't defined in some browsers.		
...
//A fucntion that generates a unique id.
		guid: function(prefix) {
			return (prefix || "obj_")+ new Date().getTime();
		},

//A function that fatches a random element.	
...
//NI Constants- varaibles that are intended to only be read.
	NI.co = {
