...
(function(tc) {
	...
	tc.particle.particle = function(options){
		...
		_me.reset_forces = function reset_forces(){
			frc.setElements([0, 0]);
			damping = _me.options.damping;
		}
		
		_me.add_forces = function add_forces(forces){
		...
		_me.bounce_off_walls = function bounce_off_walls(bounds){
			var b_did_i_collide;
		...
		_me.handle_anchor = function handle_anchor(){
		...
		_me.collide_with_particles = function collide_with_particles(particles,j){
		...
		_me.add_damping = function add_damping(){
			vel = vel.multiply(damping);
		}
		
		_me.update = function update(){
			vel = vel.add(frc);
			...
			//_me.pos.elements[0] = Math.floor(_me.pos.elements[0]);
			//_me.pos.elements[1] = Math.floor(_me.pos.elements[1]);
