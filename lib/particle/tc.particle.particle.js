	...
	tc.particle.particle = function(options){
		...
		this.initialize = function(){
			...
			*/
			...
			_me.pos = new tc.vector_2d([o.pos.x, o.pos.y]);
			vel = new tc.vector_2d([o.vel.x, o.vel.y]);
			frc = new tc.vector_2d([o.frc.x, o.frc.y]);
			_me.anchor = new tc.vector_2d([
				(0+o.anchor.x+o.anchor_offset.x),
				(0+o.anchor.y+o.anchor_offset.y)
			]);
		...
		_me.reset_forces = function particle_reset_forces(){
			frc.setElements([0, 0]);
			damping = _me.options.damping;
		};
		
		_me.add_forces = function particle_add_forces(forces){
			var distance, length, pct, normal_distance;
			for(var i = 0; i < forces.length; i++){
				distance = _me.pos.subtract(forces[i].pos);
				//length = Math.sqrt(distance.dot(distance));
				length = distance.dot(distance);
				if(length < (forces[i].radius * forces[i].radius)){
					pct = (1 - (length / (forces[i].radius * forces[i].radius))) * 20/o.radius;
		...
		_me.bounce_off_walls = function particle_bounce_off_walls(bounds){
			var b_did_i_collide;
		...
		_me.handle_anchor = function particle_handle_anchor(){
			...
			frc = frc.subtract([(normal_distance.elements[0] * 10 * pct), normal_distance.elements[1] * 10 * pct])
		}
		
		_me.collide_with_particles = function particle_collide_with_particles(particles,j){
			for(var i = 0; i < particles.length; i++){
				if(i != j){
					var distance = _me.pos.subtract(particles[i].pos);
					//var length = Math.sqrt(distance.dot(distance));
					var length = distance.dot(distance);
		...
		_me.stop = function particle_stop(){
			frc.setElements([0, 0])
			vel.setElements([0, 0])
		}
		
		_me.add_damping = function particle_add_damping(){
			vel = vel.multiply(damping);
		}
		
		_me.update = function particle_update(){
			vel = vel.add(frc);
			_me.pos = _me.pos.add(vel);
		}

		_me.has_changed = true;
		
		_me.draw = function particle_draw(context){
