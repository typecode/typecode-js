...
(function(tc) {
	...
	tc.particle.context = function(dom,options){
		...
		_me.start = function(){
			...
			if(_me.stopped){
				_me.stopped = false;
				//$(window).bind('mousemove',_me.update);
				_me.update();
				//_draw();
		...
		function _draw(callback){
			...
			if(_me.n_context_rendered < _me.n_context_drawn-1){
				return;
			}
			...
			//
			//my_context.fill();
			//my_context.lineWidth = 0;
			//my_context.stroke();

			_me.n_context_drawn++;
			//window.setTimeout(_draw, 5);
			window.setTimeout(_me.update, 5);
		};
		...
		function _render(){
			...
			if(_me.n_context_drawn > _me.n_context_rendered){
