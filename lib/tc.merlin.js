	...
	function Merlin(options) {
		...
		function show_step(step, nav_data){
			...
			if (o.transition=='fade') {
				...
				viewport = $( '#merlin-viewport' );
				step_width = viewport.width();
				
				o.$e.children('.step').width( step_width );
				o.$e.width( step_width * 3 ).css({ 'margin-left' : -step_width });

				o.$e.find(internal.current_step.selector).css({ left : step_width });
			
				if (internal.counters.n_times_step_rendered !== 0) {
					if ( internal.current_step.step_number > internal.last_step.step_number ) {
						o.$e.find(internal.current_step.selector).css({ left : (step_width * 2) }).show();
						o.$e.animate({ 'margin-left' : -step_width * 2 }, o.transition_speed, 'easeInOutQuad', 
							function(){
								o.$e.find(internal.last_step.selector).hide();
								o.$e.css({ 'margin-left' : -step_width });
								o.$e.find(internal.current_step.selector).css({ left : step_width });
							}
						);
					} else if ( internal.current_step.step_number < internal.last_step.step_number ) {
						o.$e.find(internal.current_step.selector).css({ left : 0 }).show();
						o.$e.animate({ 'margin-left' : 0 }, o.transition_speed, 'easeInOutQuad', 
							function(){
								o.$e.find(internal.last_step.selector).hide();
								o.$e.css({ 'margin-left' : -step_width });
								o.$e.find(internal.current_step.selector).css({ left : step_width });
