...
(function(window, $) {
	...
	function Merlin(options) {
		var o, internal, control_handlers, input_handlers;
			...
			for(i in o.steps){
				if(o.steps[i].selector && o.$e){
					...
					if(o.steps[i].inputs){
						...
						o.steps[i].$e.bind('managerValidationPass', {}, function(e,d){ return; })
							.bind('managerValidationFail', {}, function(e,d){ return; });
		...
		function show_step_old(step,force){
			...
			if(tc.jQ.isFunction(this.current_step.init)){
				this.current_step.init(this,this.current_step.$e);
			}
			this.validate(false);
