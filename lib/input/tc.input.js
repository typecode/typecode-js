...
(function(window, $) {
	...
	function Input(options) {
		...
		function init(me){
			...
			if(internal.$e.parent().hasClass('tc-input')){
				internal.wrapper = internal.$e.parent();
			} else {
				internal.wrapper = $('<div class="tc-input">');
		...
		this.set_val = function(new_val,prevent_event){
			switch (internal.$e.get(0).nodeName.toLowerCase()) {
				case "input":
					switch (internal.$e.attr("type")) {
						case "radio":
