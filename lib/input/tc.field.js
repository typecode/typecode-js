	...
	field.synthesize({
		...
		proto: {
			...
			move_selection: function(direction) {
				var me, $t, $items, $active_item, filter;
				me = this;
				...
				switch (direction) {
					case NI.co.directions.UP:
						if ($active_item) {
							$t = $active_item.prevAll(filter).eq(0);
							if (!$t.length) {
								$t = this.internal.o.circular ? $items.eq($items.length - 1) : $items.eq(0);
							}
						} else {
							$t = $items.eq($items.length - 1);
						}
						break;
					case NI.co.directions.DOWN:
					default:
						if ($active_item) {
							$t = $active_item.nextAll(filter).eq(0);
							if (!$t.length) {
								$t = this.internal.o.circular ? $items.eq(0) : $items.eq($items.length - 1);
							}
						} else {
							$t = $items.eq(0);
						}
			...
			filter: function(text) {
				...
				$items = this.get_items(function() {
					var $item = $(this);
					return ($item.hasClass(me.internal.o.filtered_item_class) || !($item.css("display") === "none"));
				});
				if (!$items.length) {
					return this;
				}
