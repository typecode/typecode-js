	...
	field.module({
		...
		proto: {
			...
			generate_from_dom: function(options) {
				...
				$items = this.$c.find("."+ this.internal.o.item_class);
			...
			generate_from_scratch: function(options) {
				this.$c = $("<div></div>")
					...
					.html("<div class='"+ this.internal.o.display_class +"'></div>"+
					      "<ul class='"+ this.internal.o.menu_class + "'></ul>");
				this.elements.display = $("."+ this.internal.o.display_class, this.$c);
				this.elements.menu = $("."+ this.internal.o.menu_class, this.$c);
				if (options.value_list) {
					this.populate(options.value_list);
			...
			get_active_item: function() {
				var $item;
				$item = this.elements.menu.children("."+ field.co.ACTIVE_CLASS);
			...
			_set_val: function(val) {
				...
				this.elements.menu.children().each(function(i) {
					var $item = $(this);
					if ($item.attr(field.co.VALUE) == val) {
						$item.addClass(field.co.ACTIVE_CLASS).siblings().removeClass(field.co.ACTIVE_CLASS);
			...
			move_selection: function(direction) {
				...
				$items = this.elements.menu.children("."+ this.internal.o.item_class);
				if (!$items.length) {
					return this;
				}
			...
			add_item: function() {
				...
				if (arguments[0] instanceof $) {
					...
					$item = $("<li "+ field.co.VALUE +"='"+ arguments[0] +"'>"+ arguments[1] +"</li>");
				}
			...
			find: function(val) {
				var $t;
				$t = false;
				this.elememts.menu.children("."+ this.internal.o.item_class).each(function() {
					var $item = $(this);
					if ($item.attr(field.co.VALUE) == val) {
						$t = $item;
						return false;
