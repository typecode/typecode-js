...
define(['jquery', 'NIseed'], function($) {
	...
	function Merlin(options) {
		var me, o, internal, control_handlers, step_input_handlers, step_event_handlers;
		...
		o = $.extend({},{
			...
			validation_watch_events: ['keyup', 'change'],
			
			transition:'none', // fade, slide, horiz_slide, none
		...
		function initialize_steps(me){
			var i, e_data;
			$.each(o.steps,function(i){
				var k;
				if(o.steps[i].$e || (o.steps[i].selector && o.$e)){
					o.steps[i].name = i;
					...
					o.steps[i].initialize_fields = function(){
						if(o.steps[i].fields){
							...
							require(['NIvalidation'], function() {

								o.steps[i].validationManager = new NI.ValidationManager({
									$mother:o.steps[i].$e,
									watchEvents: o.validation_watch_events
								});
		...
		function show_step(step, nav_data){
			...
			var i, strip, viewport, step_width;
			...
			if (o.transition=='fade') {
				if (internal.counters.n_times_step_rendered !== 0) {
					internal.current_step.$e.fadeIn(o.transition_speed);
					internal.last_step.$e.fadeOut(o.transition_speed);
				} else {
					toggle_step_visibility(internal.current_step.name);
				}
			} else if (o.transition=='slide') {
				if (internal.counters.n_times_step_rendered !== 0) {
					internal.last_step.$e.slideUp(o.transition_speed/2);
					internal.current_step.$e.delay(o.transition_speed/2).slideDown(o.transition_speed);
				} else {
					toggle_step_visibility(internal.current_step.name);
				}
				...
				if (internal.counters.n_times_step_rendered !== 0) {
					...
					$.each(internal.steps, function() {
						this.$e.width(step_width);
					});

					strip.width( step_width * 2);
					internal.current_step.$e.show();
		...
		function toggle_step_visibility(step) {
			$.each(internal.steps, function() {
				if (this.name == step) {
					this.$e.show();
				} else {
