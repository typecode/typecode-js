...
//instance is the instance of Carousel.
	...
	function Carousel(options) {
		...
		o = $.extend({
			...
			cloneClass: "clone",
			speed: 400,
			easing: "swing",		// How many different effects are there?
			vertical: false,
			circular: true,
			keyboard: true,
		...
		function init() {
			...
			$elements = {
				...
				prevBtn: null,		//can I put generate.prevBtn(me)?
				nextBtn: null
			};
		//If the dimensions of the viewport is explicitly told, not using css, this will
		...
		this.remove = function($panel) {
			...
			if (!($.contains($elements.scroll[0], $panel[0])) || $panel.hasClass(o.cloneClass)) {
				//*********************
				if($panel.hasClass(o.cloneClass) && $panel(this.info().current - 1).hasClass(o.cloneClass)){
					return false;
				}
				//***********************
			}
			...
			//****
			previousIndex = info.$current.index() - 1;
			//****
