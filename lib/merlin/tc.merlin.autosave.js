	...
	function MerlinAutosave(options){
		...
		this.init = function(me,functions){
			...
			for(i in me.internal.current_step.inputs){
				...
				console.log(me.internal.current_step.inputs[i].component.get_dom());
				
				me.internal.current_step.inputs[i].component.get_dom().bind('change keyup',{ name:i },function(e,d){
					...
					if( (!me.get_data()[e.data.name] && (me.get_data()[e.data.name].length !== 0) ) || (me.get_data()[e.data.name] == me.internal.current_step.inputs[e.data.name].component.get_val()) ){
						return;
					}
