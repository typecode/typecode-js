...
define(['jquery', 'NIseed'], function($) {
	...
	function Carousel(options) {
		...
		// assumes arguments are class selectors specified without a '.'
		...
		function init() {
			...
			$viewport = $e.find('.' + o.viewport_class);

			$elements = {
				viewport: $viewport,
				scroll: $viewport.find('.' + o.scroll_class),
				current_target: null,
				prev_btn: null,
				next_btn: null
			};
		...
		function current() {
			return $elements.scroll.find(selectors.panel + selectors.active);
		}

		function target_position($panel) {
			if (o.vertical) {
				if (o.viewport_dimensions && typeof o.viewport_dimensions.height === 'number') {
					//return {top: -($panel.index()*o.viewport_dimensions.height) + o.offset};
					return {top: -($panel.index()*o.viewport_dimensions.height)};
				}
				//return {top: -($panel.position().top) + o.offset};
		...
		this.info = function() {
			var $panels, $current, $current_clone, $next, $prev,index;
			$panels = $elements.scroll.children(selectors.panel).not(selectors.clone);
			$panels_with_clones = $elements.scroll.children(selectors.panel);
			...
			return {
				...
				total: $panels.length,
				$panels: $panels,
				$panels_with_clones: $panels_with_clones
			};
		};
